...
Um eine Fehlerabschätzung für die Mehrdimensionalen Quadratur zu erhalten geht man
eben so wie das Verfahren mit Hilfe von Fubini vor.

Geht man nun von einer $d$ dimensionalen Funktion aus, mit eindimensionalen Quadraturen mit Anzahl der Stützstellen $n_d$ so
...
(Vorrausgesetzt natürlich $g_k^i\le 1 \forall i, k$).
...
Man sieht also, dass sich die Fehler im schlimmsten Fall aufsummieren. Wählt man für alle Dimensionen das selbe Verfahren selber Ordnung,
...
der Intervalllänge, der Anzahl der Stützstellen n und höheren Ableitungen der Funktion abhängt. 
...
Hier haben wir eine lineare Funktion (die Summe der Koordinaten) auf den
...
\begin{spverbatim}
octave:24> for i = 1:5, tic; mulDimIntGauss(@(x) sum(x), zeros(i,1), ones(i,1), 1e-4); toc; end
...
Mit dem adaptiven Integrator beobachten wir einen deutlichen größeren Faktor:

\begin{spverbatim}
octave:25> for i = 1:2, tic; mulDimIntOpt(@(x) sum(x), zeros(i,1), ones(i,1), 1e-4); toc; end
...
\begin{spverbatim}
octave:17> for i = 4:15, tic; mulDimIntGauss(@(x) exp(x(1)*x(2)*x(3)), [0 0 0], [1 1 1], 10^-i); toc; end
...
\begin{spverbatim}
octave:19> for i = 4:9, tic; mulDimIntOpt(@(x) exp(x(1)*x(2)*x(3)), [0 0 0], [1 1 1], 10^-i); toc; end
